...
class Product:
    async def __init__(self, id: str, name: str, tagline: str, description: str, votesCount: int, createdAt: str, featuredAt: str, website: str, url: str, **kwargs):
        ...
        self.url = url
        self.og_image_url = await self.fetch_og_image_url()
    ...
    async def fetch_og_image_url(self) -> str:
    ...
    async def generate_keywords(self) -> str:
        ...
        try:
            ...
            response, _ = await call_dify_app(DIFY_API_KEY, prompt, "", inputs, "", "blocking")
    ...
    async def translate_text(self, text: str) -> str:
        ...
        try:
            ...
            response, _ = await call_dify_app(DIFY_API_KEY, text, "", inputs, "", "blocking")
...
async def get_producthunt_token():
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, json=payload, headers=headers) as response:
            if response.status != 200:
                raise Exception(f"Failed to obtain access token: {response.status}, {await response.text()}")
    ...
    async with aiohttp.ClientSession() as session:
        while has_next_page and len(all_posts) < 30:
            ...
            async with session.post(url, headers=headers, json={"query": query}) as response:
                if response.status != 200:
                    raise Exception(f"Failed to fetch data from Product Hunt: {response.status}, {await response.text()}")

                data = (await response.json())['data']['posts']
                posts = data['nodes']
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, headers=headers, json=payload) as response:
            ...
            if response_mode == "blocking":
                return await handle_blocking_response(response, conversation_id)
            elif response_mode == "streaming":
                return await handle_streaming_response(response, conversation_id)

async def handle_blocking_response(response: aiohttp.ClientResponse, conversation_id: str) -> Tuple[str, str]:
    try:
...
async def handle_streaming_response(response: aiohttp.ClientResponse, conversation_id: str) -> Tuple[str, str]:
    result = ""
